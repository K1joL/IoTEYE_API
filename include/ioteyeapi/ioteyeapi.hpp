...
#include "ioteyeapi/ioteyeapi_export.hpp"
...
public:
  ...
  auto name() const -> char const*;
   ...
   /**
    *   @brief Фукнция отправки запроса
...
private:
  IOTEYEAPI_SUPPRESS_C4251
  std::string m_name;

  std::string G_USERID{};
  std::string s_ENDPOINT_USERID{};
  std::unordered_map<std::string, uint64_t> s_devices;
  std::unordered_map<std::string, uint8_t> s_devicesStatus;
};
