#include "ioteye_payload.hpp"

namespace ioteye {
PairString::PairString() : key{}, value{} {
}

PairString::PairString(const char *key, const char *value) {
    this->key = key;
    this->value = value;
}

PairString::PairString(const char *key, const String &value) {
    this->key = key;
    this->value = value;
}

PairString::PairString(const String &value, const char *key) {
    this->key = key;
    this->value = value;
}

PairString::PairString(const PairString &pairString) {
...
Payload::Payload(const PairString &pair) {
    m_pairs = new PairString[m_capacity];
    m_pairs[m_size] = pair;
...
void Payload::Add(const PairString &pair) {
...
String Payload::GetJsonString() const {
    String jsonString('{');
...
String Payload::GetArgsString() const {
    String args{};
    // Рассчитать емкость, чтобы избежать многократного перераспределения
...
PairString &Payload::operator[](unsigned int index) {
    if (!m_size)
...
const PairString &Payload::operator[](unsigned int index) const {
    if (!m_size)
        return this->m_pairs[0];
